...
def test_append_shared_null_range_preserves_logical_tail_allocation() -> None:
    block_size = 16
    ...
    block_pool = BlockPool(
        num_gpu_blocks=16, enable_caching=False, hash_block_size=block_size
    )
    ...
    request_id = "request"
    prefix_blocks = block_pool.get_new_blocks(2)
    manager.req_to_blocks[request_id].extend(prefix_blocks)
    manager.num_cached_block[request_id] = 0

    manager.append_shared_null_range(request_id, start_token=32, end_token=64)
    assert manager.req_to_blocks[request_id][2:] == [block_pool.null_block] * 2

    new_blocks = manager.allocate_new_blocks(
        request_id, num_tokens=80, num_tokens_main_model=80
    )
    assert len(new_blocks) == 1
    assert len(manager.req_to_blocks[request_id]) == 5
    assert not manager.req_to_blocks[request_id][-1].is_null


def test_append_shared_null_range_rejects_non_tail_or_unaligned_range() -> None:
    block_size = 16
    spec = FullAttentionSpec(
        block_size=block_size,
        num_kv_heads=1,
    ...
    manager.req_to_blocks["request"].extend(block_pool.get_new_blocks(2))

    with pytest.raises(ValueError, match="block-aligned"):
        manager.append_shared_null_range("request", 32, 63)
    with pytest.raises(ValueError, match="block-table tail"):
        manager.append_shared_null_range("request", 16, 48)


def test_pop_shared_null_range_restores_original_tail() -> None:
    block_size = 16
    spec = FullAttentionSpec(
        block_size=block_size,
        num_kv_heads=1,
    ...
    prefix_blocks = block_pool.get_new_blocks(2)
    manager.req_to_blocks["request"].extend(prefix_blocks)
    manager.num_cached_block["request"] = 2
    manager.append_shared_null_range("request", 32, 64)

    manager.pop_shared_null_range("request", 32, 64)

    assert manager.req_to_blocks["request"] == prefix_blocks
    assert manager.num_cached_block["request"] == 2
    with pytest.raises(ValueError, match="block-table tail"):
        manager.pop_shared_null_range("request", 32, 64)
